        ...
        for epoch in range(self.epochs):
            ...
            for iter, (x_, y_) in enumerate(self.training_set_data_loader):
                ...
                if ((iter + 1) % self.n_critic) == 0:
                    ...
                    self.train_hist['G_loss'].append(D_real_loss-torch.mean(D_fake))
...
if __name__ == '__main__':
    ...
    dtype = torch.float
    percent=0.05
    root_dir='./original_data_wgan_data_'+str(percent)+'percent_'+ time.strftime("%Y%m%d-%H:%M:%S", time.localtime())
